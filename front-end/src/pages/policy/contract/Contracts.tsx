import { Button, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Collapse, IconButton } from "@mui/material";
...
const Contracts = () => {
    ...
    if (creating) {
        return <div className="flex justify-center items-center h-[800px]"><NewContract onCancel={() => setCreating(false)} /></div>;
    }
                        ...
                        <React.Fragment key={contract.id}>
                            <TableRow>
                                <TableCell>
                                    ...
                                    >
                                        {expandedRow === contract.id ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                                    </IconButton>
                                </TableCell>
                                ...
                                <TableCell align="right">{contract.effectiveDate?.toString()}</TableCell>
                                <TableCell align="right">{contract.expirationDate?.toString()}</TableCell>
                            ...
                            <TableRow>
                                <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={8}>
                                    <Collapse in={expandedRow === contract.id} timeout="auto" unmountOnExit>
                                        <div className="p-4">
                                            ...
                                            {contract.claims && contract.claims.length > 0 ? (
                                                    ...
                                                    <TableBody>
                                                        {contract.claims.map((claim) => (
                                                            <TableRow key={claim.id}>
                                                                ...
                                                                <TableCell align="right">{claim.date?.toString()}</TableCell>
                                                                <TableCell align="right">{claim.description}</TableCell>
                                                ...
                                                <p className="text-gray-500">No claims found for this contract.</p>
                                            )}
                                        </div>
                                    </Collapse>
                                </TableCell>
